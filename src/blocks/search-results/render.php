...
$capitola_search_string = get_query_var( 's', '' );
	...
	<div class="capitola-listings__width alignwide">
		<div class="capitola-listings__search-head">
			...
			<form class="capitola-listings__search-form" action="/" autocomplete="off">
				<input type="search" name="s" value="<?php echo esc_attr( $capitola_search_string ? sanitize_text_field( $capitola_search_string ) : '' ); ?>" aria-label="search" placeholder="What are you looking for?"/>
				<button type="submit" class="search-icon" aria-label="submit search"></button>
			</form>
			<div class="capitola-listings__search-count">
				We found <?php echo esc_html( $wp_query->found_posts . ' ' . $capitola_not_singular ); ?> in your search.
			</div>
		</div>
