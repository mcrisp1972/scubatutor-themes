			...
			<div className="capitola-result__image-col --theme-image-overlay">
				...
				<div className="capitola-result__thumb-content">
					{ conditionals.titleLocation === 'image' && (
						<>
							<div className="capitola-result__thumb-title --hl-s">{ post.title.raw }</div>
							{ post.event_dates && (
								<div className="capitola-result__thumb-subtitle">{ post.event_dates }</div>
							) }
						</>
					) }
					{ attributes.ctaText && conditionals.ctaLocation === 'image' && (
						<span className="capitola-result__thumb-cta --cta --tertiary">{ attributes.ctaText }</span>
					) }
				</div>
				{ conditionals.titleLocation === 'image' && post.category_name && (
					<div className="capitola-result__thumb-cat">{ decodeEntities( post.category_name ) }</div>
				) }
			</div>
				...
				<div className="capitola-result__content">
					{ conditionals.titleLocation === 'body' && (
						<div>
							...
							{ post.event_dates && <div className="capitola-result__subtitle">{ post.event_dates }</div> }
						</div>
					) }
					...
					{ attributes.ctaText && conditionals.ctaLocation === 'body' && (
						<div className="capitola-result__cta --cta --tertiary">{ attributes.ctaText }</div>
					) }
				</div>
			) }
		</div>
