...
<section <?= wp_kses_data( $wrapper_attributes ) ?>>
	<?php if ( $attributes['showFeaturedImage'] && $image_id ) : ?>
		<div class="wp-block-cwps-post-hero__hero">
			<div class="wp-block-cwps-post-hero__image">
				<?= wp_get_attachment_image( $image_id, 'large', false, array( 'loading' => 'eager' ) ) ?>
			</div>
		</div>
	<?php endif; ?>
	<<?= tag_escape( $attributes['headlineTag'] ) ?> class="wp-block-cwps-post-hero__title --hl-xl">
		<?= esc_html( $attributes['headline'] ? $attributes['headline'] : get_the_title() ) ?>
	</<?= tag_escape( $attributes['headlineTag'] ) ?>>
	<div class="wp-block-cwps-post-hero__details">
		<div class="wp-block-cwps-post-hero__byline">
		<?php if ( get_post_type() === 'post' ) : ?>
			<?php if ( $attributes['showByline'] ) : ?>
				<?php if ( $author_image_src ) : ?>
					<div class="wp-block-cwps-post-hero__byline-img-wrap">
						<img src="<?= esc_url( $author_image_src[0] ) ?>" alt="<?= esc_html( $name ) ?>"/>
					</div>
				<?php endif; ?>
				<div class="wp-block-cwps-post-hero__byline-date">
		...
		<?php if ( $attributes['showSocials'] ) : ?>
			<ul class="wp-block-cwps-post-hero__social-links">
				<?php
				foreach ( $socials as $k => $v ) :
					...
					if ( $social_link ) :
						?>
						<li>
							<a class="wp-block-cwps-post-hero__social-link --<?= esc_attr( $k ) ?>" href="<?= esc_url( $social_link ) ?>" target="_blank" aria-label="<?= esc_attr( $k ) ?>"></a>
						</li>
					<?php endif; ?>
				<?php endforeach; ?>
			</ul>
