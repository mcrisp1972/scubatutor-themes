...
|------|------|---------|-------------|
| `props` | `Object` | - | **Required.** Block props containing `attributes` and `setAttributes` |
| `attribute` | `string` | - | **Required.** The attribute name for the repeater field |
| `index` | `number` | - | **Required.** The current row index |
| `newValues` | `any` | - | **Required.** Default values for new rows |
| `fullButton` | `boolean` | `false` | Whether the trigger button should fill the entire parent container. This is the best option when all values will be set inside the popover instead of inline editing using RichText. |
| `vertical` | `boolean` | `false` | Whether to use vertical icons for the move buttons (up/down vs left/right) |
| `onImageChange` | `Function` | `false` | Callback when an image is selected |
| `imageValue` | `integer` | `false` | Current image ID if the onImageChange prop is set |
| `onAddAfter` | `Function` | `false` | Callback when a row is added after the current one |
| `allowNull` | `boolean` | `false` | Whether to allow deleting the last remaining row |
| `style` | `Object` | `{}` | Custom styles for the trigger button. Especially useful for size and positioning. |
| `children` | `ReactNode` | `null` | Custom content to display in the popover |
