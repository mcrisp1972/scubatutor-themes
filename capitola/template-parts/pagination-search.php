...
$search_string     = sanitize_text_field( get_query_var( 's', 1 ) );
...
<nav class="capitola-page-nav js-pageNav">
	...
	<ul class="capitola-page-nav__page-numbers js-navPageNumbers">
		...
		foreach ( $pagination_values as $p ) :
			...
			<?php else : ?>
				<li>
					<a href="<?php echo esc_url( '/page/' . $p . '/?s=' . rawurlencode( $search_string ) ); ?>" class="capitola-page-nav__button --number <?php echo ( $p === $current_page ? '--current' : '' ); ?>" data-page="<?php echo esc_attr( $p ); ?>"><?php echo esc_html( $p ); ?></a>
				</li>
				<?php
			endif;
		endforeach;
