		...
		<div class="wp-block-capitola-nav__grid alignwide">
			<?php if ( ! empty( $logo_object ) ) : ?>
				<?php if ( str_starts_with( $logo_object['sizes']['medium']['mime-type'], 'image/svg' ) && ! $attributes['useLogoColor'] ) : ?>
					<a class="wp-block-capitola-nav__logo --has-svg-mask js-trapMobile" href="<?php echo esc_url( get_home_url() ); ?>" style="aspect-ratio: <?php echo esc_attr( $logo_object['width'] . '/' . $logo_object['height'] ); ?>;--mask-image: url(<?php echo esc_url( wp_get_attachment_image_url( $attributes['logo'], 'medium' ) ); ?>);" aria-label="home">
						<img src="<?php echo esc_url( wp_get_attachment_image_url( $attributes['logo'], 'medium' ) ); ?>" class="custom-logo" alt="<?php echo esc_attr( get_bloginfo( 'name', 'display' ) ); ?>" height="<?php echo esc_attr( $logo_object['height'] ); ?>" width="<?php echo esc_attr( $logo_object['width'] ); ?>"/>
					</a>
				<?php else : ?>
					...
					<?php foreach ( $attributes['utilityLinks'] as $item ) : ?>
						<?php if ( ! empty( $item['link']['url'] ) ) : ?>
							<li class="wp-block-capitola-nav__utility-menu-item">
								<a class="wp-block-capitola-nav__utility-menu-item-link" href="<?php echo esc_url( $item['link']['url'] ); ?>"<?php echo ( ! empty( $item['link']['opensInNewtab'] ) ? ' target="_blank"' : '' ); ?>><?php echo esc_html( $item['title'] ); ?></a>
							</li>
						<?php endif; ?>
					<?php endforeach; ?>
					<?php if ( $attributes['showPhoneLink'] ) : ?>
						<li class="wp-block-capitola-nav__utility-menu-item">
							<a href="tel:<?php echo esc_attr( phone_link_number( $contact_phone ) ); ?>" class="wp-block-capitola-nav__utility-menu-item-link --phone"><?php echo esc_html( $contact_phone ); ?></a>
						</li>
					<?php endif; ?>
					<?php if ( $attributes['showAccountIcon'] && CAPITOLA_WOO_ACTIVE ) : ?>
						<li class="wp-block-capitola-nav__utility-menu-item">
							<a href="<?php echo esc_url( $myaccount_page_url ); ?>" class="wp-block-capitola-nav__utility-menu-item-link --account">Account</a>
						</li>
					<?php endif; ?>
